...
class Pubg {
  ...
  factory Pubg.fromJson(Map<String, dynamic> jsondata) {
    return Pubg (
      id: jsondata['id'],
...
class PubData extends StatefulWidget {
  @override
  _PubDataState createState() => _PubDataState();
}

class _PubDataState extends State<PubData> {

  final String url = "http://192.168.43.106/api/pubg.php";
  List data;
              ...
              return Padding(
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    Pubg pubg=snapshot.data[index];
                    return ListTile(
                      leading: CircleAvatar(child: Image (image: new AssetImage("assets/img/pubgicon.jpg")),backgroundColor: Color(0xFF191919)),
                      title: Text(pubg.start_time,style: TextStyle(color: Colors.white,fontWeight: FontWeight.bold,fontSize: 20.0),),
                      subtitle: Text(pubg.heading,style: TextStyle(color: Color(0xFFE0F2F1),fontSize: 17.0)),
                      onTap: () {
                        Navigator.push(context, MaterialPageRoute(builder: (context) => PubgDetail(detail: pubg,)));
                      },
                    );
                  },
  ...
  Future<List<Pubg>> downloadJSON() async {
    final jsonEndpoint = "http://192.168.43.106/api/pubg.php";
    final response = await get(jsonEndpoint);
    if (response.statusCode == 200) {
      List pubg = jsonDecode(response.body);
      print(pubg.toString());
      return pubg.map((pubg) => new Pubg.fromJson(pubg)).toList();
    } else
...
class PubgDetail extends StatefulWidget {
  Pubg detail;

  PubgDetail({this.detail});

  @override
  _PubgDetailState createState() => _PubgDetailState();
}

class _PubgDetailState extends State<PubgDetail> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Color(0xFF191919),
